...
//onHomepage
exports.getProductsOnHomepage = catchAsyncError(async (req, res, next) => {
	const products = await Product.find({ onHomepage: { $ne: "none" } }).select(
		"name images weight_of_the_commodity avg_rating productTitle categories total_no_of_reviews onHomepage description"
	);
	//   const products1 = await Product.find({});
	//   products1.forEach(async (product) => {
	//     product.onHomepage = "none";
	//     await product.save();
	//   });
...
exports.getAllProducts = catchAsyncError(async (req, res, next) => {
	const products = await Product.find().select(
		"name images weight_of_the_commodity avg_rating productTitle categories total_no_of_reviews"
	);
	res.status(200).json({
		success: true,
		products,
...
exports.getProducts = catchAsyncError(async (req, res, next) => {
	...
	// console.log(resPerPage, pageNumber, filters);

	let countQuery = Product.find();

	if (filters.length > 0) {
		countQuery = countQuery.where("categories.subcategoryids").in(filters);
		// countQuery = countQuery.where("categories.subcategoryids").all(filters);
	}

	const totalProduct = await countQuery.countDocuments();

	// console.log(resPerPage, pageNumber, totalProduct);

	let query = Product.find().select(
		"images weight_of_the_commodity avg_rating productTitle total_no_of_reviews description"
	);

	if (filters.length > 0) {
		//select all products that have any one of the filters
		query = query.where("categories.subcategoryids").in(filters);
		// query = query.where("categories.subcategoryids").all(filters);
	}
...
exports.getProductNames = catchAsyncError(async (req, res, next) => {
	const products = await Product.find().select(" _id productTitle");
	...
	logger.info("get product names and ids");
});
...
exports.showAllProducts = catchAsyncError(async (req, res, next) => {
	try {
		...
		const medicalCareFilter = medicalCareId
			? { medicalCare: medicalCareId }
			: {};
